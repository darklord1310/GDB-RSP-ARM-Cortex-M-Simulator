...
//    Vector setupList;
	...
	chipMenu.add(getClassCheckItem("Add Multiplexer", "MultiplexerElm"));
        chipMenu.add(getClassCheckItem("Add 2 to 1 Multiplexer", "Multiplexer2To1Elm"));
        chipMenu.add(getClassCheckItem("Add ARM Simulator", "ARMSim"));
	...
	for (iter = 1; ; iter++) {
		...
		if (circuitNonLinear) {
			...
			for (j = 0; j != circuitMatrixSize; j++)
                ...
                // run each of the circuit elements for doStep
		for (i = 0; i != elmList.size(); i++) {
		    CircuitElm ce = getElm(i);
		    ce.doStep();
		}
		...
		for (j = 0; j != circuitMatrixFullSize; j++) {
			...
			CircuitNode cn = getCircuitNode(j+1);
                        // run each of the circuit elements for calculate current
